...
from database import SessionLocal, Usuarios, Categorias, Productos, Inventarios, Movimientos, Clientes, Proveedores, Ventas, Compras, Auditorias
...
def generar_nuevo_id(session, modelo, campo_id: str, prefijo: str, separador="_", ceros=2):
...
def registrar_cliente():
    ...
    with obtener_session() as db:
        ...
        nombres = Prompt.ask("  [#7CCA62]>[/] [white]Nombre(s)[/]")
        apellidos = Prompt.ask("  [#7CCA62]>[/] [white]Apellido(s)[/]")
        telefono = Prompt.ask("  [#7CCA62]>[/] [white]Teléfono[/]")
        direccion = Prompt.ask("  [#7CCA62]>[/] [white]Dirección[/]")


        while True:
            correo = Prompt.ask("  [#7CCA62]>[/] [white]Correo electrónico[/]")
            ...
            if cliente_existente:
                ...
                break
        while True:
            usuario = Prompt.ask("  [#7CCA62]>[/] [white]Ususario: [/]")
        ...
        while True:
            contraseña = Prompt.ask("  [#7CCA62]>[/] [white]Cree una contraseña[/]", password=True)
            confirmar = Prompt.ask("  [#7CCA62]>[/] [white]Confirme su contraseña[/]", password=True)
        ...
        nuevo_cliente = Clientes(
            ...
            estado="Activo"  # ← aquí está la corrección importante
        )
        db.add(nuevo_cliente)
        db.commit()

        registrar_auditoria(db, id_cliente, 'Clientes', 'Registro de Cliente', f'Se dio de alta el cliente {Clientes.id_cliente}' )
    ...
    while True:
        ...
        if opcion == '1':
            ...
            with obtener_session() as db:
                ...
                proveedores_lista = db.query(Proveedores).all()
                        ...
                        if adm_contraseña == contraseña:
                            with obtener_session() as db:
                                ...
                                registrar_auditoria(
                                    ...
                                    entidad="proveedor",
                                    id_usuario=ID,
                                    descripcion=f"Se dio de Alta el proveedor '{proveedor_nuevo.nombres}': ID='{proveedor_nuevo.id_proveedor}', Empresa='{proveedor_nuevo.empresa}', Teléfono={proveedor_nuevo.telefono}, estado='{proveedor_nuevo.estado}'."
                                )
        ...
        if opcion == '1':
            ...
            with obtener_session() as db:
                ...
                if usuario:
                    ...
                    categoria = lista_cat(categorias_lista)
            ...
            with obtener_session() as db:
                ...
                if usuario:
                    ...
                    table01 = Table(show_header=True, header_style="bold #7CCA62", border_style="#7CCA62")
                    table01.add_column("Id Producto")
                    table01.add_column("Nombre")
                    table01.add_column("Descripción")
                    table01.add_column("Precio unitario")
                    table01.add_column("Categoria")
                    table01.add_column("Estado")

                    for producto in productos:
                        table01.add_row()
                        table01.add_row(producto.id_producto, producto.nombre, producto.descripcion, str(producto.precio_unitario), categoria.nombre, producto.estado)
                    ...
                    while True:
                        ...
                        productos_ids = {c.id_producto for c in productos}
                        ...
                        if productos_id == '?':
                            ...
                            break
                    print(f"Producto seleccionado: [bold]{producto.nombre}[/] - Precio actual: [green]${producto.precio:.2f}[/]")
                    ...
                    if confirmacion == "S":
                        ...
                        db.commit()
                        print(f"[SUCCESS] Precio actualizado de ${precio_anterior:.2f} a ${nuevo_precio:.2f} ✅")
                ...
                if opcion == "1":
                    ...
                    with obtener_session() as db:
                        ...
                        if usuario:
                            ...
                            while True:
                                ...
                                if productos_id == '?':
                                    table = lista_prod(productos_lista)
                                    print(table)
                                elif productos_id not in productos_ids:
                                    tema_advertencia.print("[ERROR \u274C ] No existe un producto activo con ese ID. Intente de nuevo.\n", style="error")
                                else:
                    ...
                    with obtener_session() as db:
                        ...
                        if usuario:
                            ...
                            print(title)
                        # (TABLA) Definición de tabla que enliste los productos registrados en la base de datos para su consulta
                            table = lista_prod(productos_inactivos)
                            while True:
        ...
        if opcion == '1':
            with obtener_session() as db:
                ...
                if usuario:
                    ...
                    id = generar_nuevo_id(db, Usuarios, "id_usuario", prefijo="USER", ceros=2)
                    tema_advertencia.print(f"[INFO \u2139 ] ID generado automáticamente: {id}\n", style="info")
                    ...
                    workstation = Prompt.ask("  [#7CCA62]>[/] [white]Puesto[/]")
                    phone = Prompt.ask("  [#7CCA62]>[/] [white]Teléfono[/]")

                    # (COPROBACIÓN) Revisa si el salario ingresado se introduce correctamente de acuerdo al tipo de dato
                    while True:
                        try:
                            salary = float(Prompt.ask("  [#7CCA62]>[/] [white]Salario[/]"))
                            break
                        except ValueError:
                            tema_advertencia.print("[ERROR \u274C ] El precio unitario debe ser un número válido. Intente nuevamente.\n", style="error")
                    
                    # (COMPROBACIÓN) Revisa si el usuario esta disponible o ya se encuentra en uso dentro de la base de datos
                    while True:
                        user = Prompt.ask("  [#7CCA62]>[/] [white]Usuario[/]")
                        user_existente = db.query(Usuarios).filter(Usuarios.usuario == user).first()
                        if user_existente:
                            tema_advertencia.print("[USUARIO NO DISPONIBLE \u274C ] El usuario ya esta en uso. Pruebe de nuevo.\n", style="error")
                            continue
                        else: break

                    password = Prompt.ask("  [#7CCA62]>[/] [white]Contraseña[/]")
                        ...
                        if adm_contraseña == contraseña:
                            with obtener_session() as db:
                                ...
                                registrar_auditoria(
                                    ...
                                    operacion="Alta",
                                    entidad="Empleado",
                                    id_usuario=ID,  # Este debe ser el ID del administrador o usuario que está realizando la acción
                                ...
                                tema_advertencia.print("[OPERACIÓN EXITOSA \u2714 ] Se dado de alta al nuevo usuario.\n", style="success")
            ...
            with obtener_session() as db:
                ...
                if usuario:
                    id_usuario = usuario.id_usuario
                    contraseña = usuario.contraseña
                    title = Panel("ELIMINAR EMPLEADO", style="bold #7CCA62", width=50)
                    print(title)
                    ...
                    while True:
                        ...
                        if datos_usuario == None:
                            ...
                            tema_advertencia.print("[ID INVÁLIDO \u274C ] No puedes darte de baja. Otro administrador debe hacerlo.\n", style="error")
                            continue
                        break

                    tema_advertencia.print("\n[ADVERTENCIA \u26A0 ] ¿Desea eliminar este usuario? S/N", style="advice")
                    opcion = input("Opcion: ").upper()

                    if opcion == 'S':
                        adm_contraseña = Prompt.ask("  [#7CCA62]>[/] [white]Digite su contraseña[/]", password = True)
                        if adm_contraseña == contraseña:
                            db.delete(datos_usuario)
                            db.commit()
                            tema_advertencia.print("[OPERACIÓN EXITOSA \u2714 ] Se ha dado de baja al usuario del sistema correctamente\n", style="success")
                        
                        else: tema_advertencia.print("[CREDENCIALES INCORRECTAS \u274C ] No se han realizado cambios.\n", style="error")
                    
                    elif opcion == 'N':tema_advertencia.print("[INFORMACIÓN \u2139] El proceso ha sido cancelado.\n", style="info")
            ...
            with obtener_session() as db:
                usuarios_lista = db.query(Usuarios).all()
                if usuarios_lista:
                    table = Table(show_header=True, header_style="bold #7CCA62", border_style="#7CCA62")
                    ...
                    table.add_column("Puesto")
                    table.add_column("Nombre(s)")
                    table.add_column("Apellido(s)")
                    table.add_column("Teléfono")
                    ...
                    for usuario in usuarios_lista:
                        ...
                        table.add_row(str(usuario.id_usuario), usuario.rol, usuario.puesto, usuario.nombres, usuario.apellidos, usuario.telefono, str(usuario.salario))
                    
                    print(title)
                    print(table)
                    print("\n")
                
                else: tema_advertencia.print("[ERROR \u274C ] No existe ningún usuario en la base de datos.\n", style="error")
...
def menu_venta_empleado(ID_empleado):
    ...
    while True:
        with SessionLocal() as db:
            id_producto = Prompt.ask("[bold cyan]Ingresa el ID del producto ('?' para ver inventario)[/]")
            if id_producto == "?":
                show_inventary()
                continue
            ...
            while True:
                try:
                    ...
                    if cantidad <= 0 or cantidad > inventario.cantidad:
            ...
            agregar_mas = Prompt.ask("[bold yellow]¿Deseas agregar otro producto? (S/N)[/]").upper()
            if agregar_mas != "S":
                break
    ...
    if Prompt.ask("[bold cyan]¿Deseas realizar esta venta? (S/N)[/]").upper() != "S":
        print("[bold yellow]Venta cancelada.[/]")
        return
...
def editar_perfil_empleado(ID_empleado):
    ...
    "[2] [white]Modificar email[/]\n"
    "[3] [white]Modificar teléfono[/]\n"
    "[4] [white]Cambiar contraseña[/]\n"
    "[5] [white]Regresar[/]", title="[bold #0BD0D9][ EDITAR PERFIL ][/]", subtitle="Ajustes de cuenta", style="#0BD0D9", width=50)
        ...
        with obtener_session() as db:
            ...
            if opcion in ['1', '2', '3']:
                if opcion == '1':
                    ...
                    nuevo_dato = Prompt.ask("  [#7CCA62]>[/] [white]Nuevo email)[/]")
                    atributo = 'email'
                elif opcion == '3':
...
def validar_sesion(db, usuario, contraseña, rol):
    usuario = db.query(Usuarios).filter(Usuarios.usuario == usuario, Usuarios.contraseña == contraseña, Usuarios.rol == rol).first()
    return usuario
...
def adm_session():
    ...
    with obtener_session() as db:
      # [ANOTACIÓN] 
        ...
        inicio_sesion = validar_sesion(db, usuario, contraseña, "Administrador")
...
def emp_session():
    ...
    with obtener_session() as db:
        # [ANOTACIÓN] 
            ...
            # si el usuario existe en el sistema. Si la validación es exitosa, los datos del usuario se guardan en la variable. En caso contrario, se muestra 
            # un mensaje de error indicando que la autenticación ha fallado.
        inicio_sesion = validar_sesion(db, usuario, contraseña, "Empleado")
...
def cli_session():
    ...
    with obtener_session() as db:
        ...
        clientes = db.query(Clientes).filter_by(usuario=usuario, contraseña=contraseña).first()
...
def cliente_comprar(cliente, carrito):
    with obtener_session() as db:
        termino = Prompt.ask("\n  [#0BD0D9]>[/] [white]Buscar producto por nombre[/]")
        productos = db.query(Productos).filter(Productos.nombre.ilike(f"%{termino}%")).all()

        if productos:
            table = Table(title="Productos Encontrados", show_header=True, header_style="bold #0BD0D9")
            table.add_column("ID")
            table.add_column("Nombre")
            ...
            for p in productos:
                inventario = db.query(Inventarios).filter_by(id_producto=p.id_producto).first()
                table.add_row(p.id_producto, p.nombre, f"${p.precio_unitario:.2f}", str(inventario.cantidad))

            print(table)
            if Prompt.ask("¿Agregar al carrito?", choices=["s", "n"]) == "s":
                id_prod = Prompt.ask("ID del producto")
                cantidad = int(Prompt.ask("Cantidad"))
                prod = db.query(Productos).filter_by(id_producto=id_prod).first()
                inv = db.query(Inventarios).filter_by(id_producto=id_prod).first()
                if prod and inv and inv.cantidad >= cantidad:
                ...
                if Prompt.ask("¿Proceder al checkout?", choices=["s", "n"]) == "s":
                    total = sum(p.precio_unitario * c for p, c in carrito)
                    tema_advertencia.print(f"Total a pagar: ${total:.2f}", style="advice")
                    if Prompt.ask("¿Confirmar compra?", choices=["s", "n"]) == "s":
                        for p, c in carrito:
                            movimiento = Movimientos(
                                id_usuario= cliente.id_cliente ,  # puedes usar cliente.id_cliente si se adapta
                                id_producto=p.id_producto,
                                descripcion="Venta al cliente",
                                cantidad=c,
                                subtotal=p.precio_unitario * c,
                                impuesto=16,
                                total=p.precio_unitario * c,
                                tipo="Venta",
                                estado="Activo"
                            )
                            db.add(movimiento)
...
def cliente_ver_inventario():
    with obtener_session() as db:
        productos = db.query(Productos).all()
        table = Table(title="Inventario", show_header=True, header_style="bold #0BD0D9")
        table.add_column("ID")
        table.add_column("Nombre")
        table.add_column("Stock")
        for p in productos:
            inv = db.query(Inventarios).filter_by(id_producto=p.id_producto).first()
            table.add_row(p.id_producto, p.nombre, str(inv.cantidad))
        print(table)
